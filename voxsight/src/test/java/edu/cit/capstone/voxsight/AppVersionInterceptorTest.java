...
class AppVersionInterceptorTest {
    ...
    void setUp() {
        ...
        properties.setMinVersionCode(3);
        properties.setMinVersionName("1.2");
    ...
    void shouldRejectRequestWhenVersionCodeIsOld() throws Exception {
        ...
        assertFalse(result, "Request with versionCode=2 (v1.1.0) must be rejected when minVersionCode=3 (v1.2)");
        assertEquals(426, response.getStatus());
        assertTrue(response.getContentAsString().contains("Update Required"));
    }
    ...
    void shouldAllowRequestWhenVersionCodeMeetsRequirement() throws Exception {
        MockHttpServletRequest request = new MockHttpServletRequest("POST", "/api/omr/analyze");
        request.addHeader(AppVersionInterceptor.HEADER_VERSION_CODE, "3");
        request.addHeader(AppVersionInterceptor.HEADER_VERSION_NAME, "1.2");
        MockHttpServletResponse response = new MockHttpServletResponse();

        boolean result = interceptor.preHandle(request, response, new Object());

        assertTrue(result, "Request with versionCode=3 (v1.2) must be allowed");
        assertEquals(200, response.getStatus());
    }
